...
class OperationManagerIntegrationTest extends \Codeception\Test\Unit
{
    use UserTrait;
    /**
     * @var \UnitTester
    ...
    public function testCreateOperationOk()
    {
        ...
        $amount = 100;
        $ok = \Yii::$container->invoke(
            [$this->operationManager, 'createOperation'],
            ['fromUser' => $user1, 'toUser' => $user2, 'amount' => $amount]
        );
        $this->assertTrue($ok);
    ...
    public function testCreateOperationError()
    {
        ...
        $ok = \Yii::$container->invoke(
            [$this->operationManager, 'createOperation'],
            ['fromUser' => $user1, 'toUser' => $user1, 'amount' => $amount]
        );
        $this->assertFalse($ok);
        $foundOp = Operation::find()
